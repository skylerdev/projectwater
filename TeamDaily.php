...
if (isset($_GET['tab']) && ($_GET['tab'] == 'personal-daily')) {
     ...
     * Returns pipe score
     *************************/
    $stmt = $conn->prepare('SELECT Score FROM users INNER JOIN pipe_score ON pipe_score.UserID = users.UserID WHERE Username = :user AND DATE = :date  ORDER BY score DESC');
     ...
     *************************/
    $stmt = $conn->prepare('SELECT Score FROM users INNER JOIN shooter_score ON shooter_score.UserID = users.UserID WHERE Username = :user AND DATE = :date  ORDER BY score DESC');
    $stmt->bindValue(':date',$date,PDO::PARAM_STR);
    $stmt->bindValue(':user', $username,PDO::PARAM_STR);
    $stmt->execute();
     ...
     * Returns Tap score
     *************************/
    $stmt = $conn->prepare('SELECT Score FROM users INNER JOIN tap_score ON tap_score.UserID = users.UserID WHERE Username = :user AND DATE = :date  ORDER BY score DESC');
    $stmt->bindValue(':date',$date,PDO::PARAM_STR);
    $stmt->bindValue(':user', $username,PDO::PARAM_STR);
    $stmt->execute();
